...
def data_generation(interactionlist):
  x = []
  edge_index = [[], []]
  edge_att = []
  y = []
  ...
  for interaction in interactionlist:
    node1 = interaction.lncRNA
    node2 = interaction.protein

    
    if node1.serial_number not in nodeSerialNumber_subgraphNodeSerialNumber_dict:  
      nodeSerialNumber_subgraphNodeSerialNumber_dict[node1.serial_number] = subgraph_node_serial_number
      subgraph_node_serial_number += 1

      vector1 = []
      for f in node1.embedded_vector:
        vector1.append(float(f))
      vector1.extend(node1.attributes_vector)
      x.append(vector1)
    
    if node2.serial_number not in nodeSerialNumber_subgraphNodeSerialNumber_dict:  
      nodeSerialNumber_subgraphNodeSerialNumber_dict[node2.serial_number] = subgraph_node_serial_number
      subgraph_node_serial_number += 1

      vector2 = []
      for f in node2.embedded_vector:
        vector2.append(float(f))
      vector2.extend(node2.attributes_vector)
      x.append(vector2)
  

    edge_index[0].append(nodeSerialNumber_subgraphNodeSerialNumber_dict[node1.serial_number])
    edge_index[1].append(nodeSerialNumber_subgraphNodeSerialNumber_dict[node2.serial_number])
    
    y.append(interaction.y) 
...
if __name__ == "__main__":
    ...
    if args.output == 1:
        if args.inMemory == 0:
            ...
            My_trainingDataset = data_generation(all_interaction_list)
            print(My_trainingDataset)

            torch.save(My_trainingDataset, './data.pt')
