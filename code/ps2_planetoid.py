...
def generate_neg_sample(split_edge, num_nodes, device, sample_size):
    ...
    edge = bipartite_negative_sampling(new_edge_index)
...
def main(data, args, device):
    loggers = {
        ...
        'PREC': Logger(args.runs, args),
        'ACC': Logger(args.runs, args),
        'REC': Logger(args.runs, args),
        'SPE': Logger(args.runs, args),
        'MCC': Logger(args.runs, args)
    }
    loggers_fine = {
        'AUC': Logger(args.runs, args),
        'AP': Logger(args.runs, args),
    ...
    for run in range(args.runs):
        ...
        results, _, _ = train_model(data, split_edge, args.batch_size, args, save_path_model_test, save_path_model, save_path_arch, device)

        results_fine, _, _ = finetune_model(data, split_edge, args.batch_size, args, save_path_model_fine, save_path_model, save_path_arch, device)
