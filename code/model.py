...
from torch_geometric.nn import GCNConv, SAGEConv, GATConv, GATv2Conv, GAE
...
class AutoLink_l3(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, num_layers,
        ...
        if self.gnn_type == 'GATV2':
            ...
            for i in range(num_layers):
                if i == 0:
                    self.convs.append(GATConv(in_channels, hidden_channels, heads=3, concat=False))
                else:
                    self.convs.append(GATConv(hidden_channels, hidden_channels, heads=3, concat=False))
